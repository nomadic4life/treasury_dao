...
describe("dao_vote_program", () => {
  ...
  it("", async () => {
    let poolState = new anchor.web3.PublicKey("HWTqMi2TMrzWqnnLj8t1yef6BiUnh7P8aY6CS5QYkK1X")

    let data = await provider.connection.getAccountInfo(poolState)

    let buf0 = Buffer.copyBytesFrom(data.data, 8 + (32 * 0), 32)
    let ammConfig = new anchor.web3.PublicKey(buf0)

    let buf1 = Buffer.copyBytesFrom(data.data, 8 + (32 * 2), 32)
    let token0Vault = new anchor.web3.PublicKey(buf1)

    let buf2 = Buffer.copyBytesFrom(data.data, 8 + (32 * 3), 32)
    let token1Vault = new anchor.web3.PublicKey(buf2)

    let buf3 = Buffer.copyBytesFrom(data.data, 8 + (32 * 5), 32)
    let token0Mint = new anchor.web3.PublicKey(buf3)

    let buf4 = Buffer.copyBytesFrom(data.data, 8 + (32 * 6), 32)
    let token1Mint = new anchor.web3.PublicKey(buf4)

    let buf5 = Buffer.copyBytesFrom(data.data, 8 + (32 * 7), 32)
    let token0Program = new anchor.web3.PublicKey(buf5)

    let buf6 = Buffer.copyBytesFrom(data.data, 8 + (32 * 8), 32)
    let token1Program = new anchor.web3.PublicKey(buf6)

    let buf7 = Buffer.copyBytesFrom(data.data, 8 + (32 * 9), 32)
    let obersvationKey = new anchor.web3.PublicKey(buf7)


    console.log(ammConfig)
    console.log(token0Vault)
    console.log(token1Vault)
    console.log(token0Mint)
    console.log(token1Mint)
    console.log(token0Program)
    console.log(token1Program)
    console.log(obersvationKey)
