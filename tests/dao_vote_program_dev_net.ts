...
const fetchPoolState = async (
    ...
    let poolState = new anchor.web3.PublicKey(key)
    let data = await connection.getAccountInfo(poolState)

    let ammConfig = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 0), 32))
    let token0Vault = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 2), 32))
    let token1Vault = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 3), 32))
    let token0Mint = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 5), 32))
    let token1Mint = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 6), 32))
    let token0Program = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 7), 32))
    let token1Program = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 8), 32))
    let obersvationKey = new anchor.web3.PublicKey(Buffer.copyBytesFrom(data.data, 8 + (32 * 9), 32))
...
describe("dao_vote_program", () => {
    ...
    let userFileInput = [
        { userFile: TEST_USERS_KEYPAIR_PATH + "/test-user-a.json" },
        { userFile: TEST_USERS_KEYPAIR_PATH + "/test-user-b.json" },
        { userFile: TEST_USERS_KEYPAIR_PATH + "/test-user-c.json" },
        { userFile: TEST_USERS_KEYPAIR_PATH + "/test-user-d.json" },
        { userFile: TEST_USERS_KEYPAIR_PATH + "/test-user-e.json" },
    ];

    let poolStateKey = [
        "HWTqMi2TMrzWqnnLj8t1yef6BiUnh7P8aY6CS5QYkK1X",
    ]

    let testUsers = []
    let poolStates = []

    before("SETTING UP TEST USERS", () => {
        userFileInput.forEach(async user => {
            let data = await fs.readFile(user.userFile, { encoding: "utf8" })
                .then(data => {
    ...
    //   describe("Treasury Member -> NEED TO ADVACE THE SLOT A SIGNIFICANT AMONT TO FULLY TEST FEATURES", () => {
